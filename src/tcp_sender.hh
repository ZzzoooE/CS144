...
class TCPSender
{
  ...
  uint64_t _RTO{};
  std::queue<TCPSenderMessage> _segments_out{};
  std::queue<TCPSenderMessage> _segments_to_send{};

//timer
  uint64_t _time_accumulate{};
  bool running{};
...
// window detail
  bool _SYN_sent{};
  bool _FIN_sent{};
  uint64_t _length_of_out{}; 
  uint64_t _lengthFilled_in_window{}; 
  uint64_t _pushed_no{};  //填充的no
  uint64_t _ackno{}; //窗口左边界
  uint64_t _final_windowSeq{1}; //窗口右边界  [)左闭右开
  uint64_t _window_size{1};  //接收方上次传来的窗口大小
...
public:
  ...
  /* Send a TCPSenderMessage if ne、eded (or empty optional otherwise) */
  std::optional<TCPSenderMessage> maybe_send();
  ...
  // void push_in_queue(TCPSenderMessage &&message);
  void stop_timer();
  void time_elapse(uint64_t ms_passed);
  void start_timer();
  // void reset_timer();
  bool timer_is_expired();
};
